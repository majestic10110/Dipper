...
class DipperModeApp:
    def __init__(self, root):
        self.root = root
        self.root.title("DIPPER V3.1 by M0OLI")
        ...
        self.rx_frame.pack(fill="both", expand=True, pady=(10, 0))  # Added pady for spacing
        ...
        self.rx_output.pack(fill="both", expand=True, pady=(0, 10))  # Increased pady below text box
    ...
    def transmit(self):
        ...
        # Insert sent text with red color and underlining
        start_pos = self.rx_output.index(tk.END)
        self.rx_output.insert(tk.END, "Sent: " + full_text + "\n", "sent")
        words = full_text.split()
        current_pos = start_pos
        for word in words:
            word_start = current_pos + " + " + str(len("Sent: ") + full_text.index(word)) + "c"
            word_end = word_start + " + " + str(len(word)) + "c"
            if word in WORD_SOUNDS:
                self.rx_output.tag_add("underline", word_start, word_end)
            current_pos = word_end + " + 1c"  # Move past the space
    ...
    def send_cq(self):
        ...
        # Insert sent CQ with red color and underlining
        start_pos = self.rx_output.index(tk.END)
        self.rx_output.insert(tk.END, "Sent: " + cq_text + "\n", "sent")
        words = cq_text.split()
        current_pos = start_pos
        for word in words:
            word_start = current_pos + " + " + str(len("Sent: ") + cq_text.index(word)) + "c"
            word_end = word_start + " + " + str(len(word)) + "c"
            if word in WORD_SOUNDS:
                self.rx_output.tag_add("underline", word_start, word_end)
            current_pos = word_end + " + 1c"  # Move past the space
